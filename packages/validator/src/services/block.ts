import {BLSPubkey, Slot, BLSSignature, allForks} from "@chainsafe/lodestar-types";
...
import {ValidatorStore, BlindedOrFullBlock, SignedBlindedOrFullBlock, BlockType} from "./validatorStore.js";
...
export class BlockProposingService {
  ...
  private async createAndPublishBlock(pubkey: BLSPubkey, slot: Slot): Promise<void> {
    ...
    try {
      ...
      const block = await this.produceBlockWrapper(slot, randaoReveal, graffiti).catch((e: Error) => {
  ...
  private publishBlockWrapper = async (signedBlock: SignedBlindedOrFullBlock): Promise<void> => {
    switch (signedBlock.type) {
      case BlockType.Blinded:
        return this.api.beacon.publishBlindedBlock(signedBlock.block);
        break;
      case BlockType.Full:
      default:
        return this.api.beacon.publishBlock(signedBlock.block);
    }
  };
  ...
  private produceBlockWrapper = async (
    ...
    graffiti: string
  ): Promise<BlindedOrFullBlock & {debugLogCtx: Record<string, string>}> => {
    const blindedBlockPromise = this.api.validator
      .produceBlindedBlock(slot, randaoReveal, graffiti)
      .catch((e: Error) => {
    ...
    if (blindedBlock) {
      ...
      return {block: blindedBlock.data, type: BlockType.Blinded, debugLogCtx};
    } else {
      ...
      // }
      return {block: fullBlock.data, type: BlockType.Full, debugLogCtx};
    }
  };
